...
use minikv::common::hash::{blake3_hash, shard_key, hrw_hash, select_replicas, ConsistentHashRing};
...
fn bench_shard_key(c: &mut Criterion) {
    ...
    for num_shards in [16, 64, 256, 1024].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(num_shards), num_shards, |b, &num_shards| {
...
fn bench_hrw_hash(c: &mut Criterion) {
    ...
    for num_nodes in [3, 5, 10, 20].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(num_nodes), &nodes, |b, nodes| {
            b.iter(|| hrw_hash(black_box("test-key"), black_box(nodes)))
        });
    }
...
fn bench_select_replicas(c: &mut Criterion) {
    ...
    for num_replicas in [1, 3, 5].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(num_replicas), num_replicas, |b, &num_replicas| {
            b.iter(|| select_replicas(black_box("test-key"), black_box(&nodes), black_box(num_replicas)))
        });
    }
